...
class InterceptorMpcDynamicsNode(Node):
    def __init__(self) -> None:
        super().__init__("interceptor_mpc_dynamics_node")

        self.declare_parameter("frame_id", "world")
        self.declare_parameter("child_frame_id", "interceptor_mpc")
        ...
        self.acceleration_command = np.zeros(3, dtype=float)
        ...
        self.state_pub = self.create_publisher(Odometry, "/interceptor_mpc/state", 10)
        self.marker_pub = self.create_publisher(Marker, "/interceptor_mpc/marker", 10)
        self.path_marker_pub = self.create_publisher(
            Marker,
            "/interceptor_mpc/path_marker",
            10,
        )
        ...
        self.get_logger().info("Interceptor MPC dynamics node started")

    def cmd_accel_callback(self, msg: Vector3) -> None:
        self.acceleration_command = np.array([msg.x, msg.y, msg.z], dtype=float)
    ...
    def timer_callback(self) -> None:
        ...
        self.state = self.dynamics.step_acceleration_command(
            state=self.state,
            acceleration_command=self.acceleration_command,
            dt=dt,
        )
        self.publish_all(now)
    ...
    def publish_path_marker(self, now: rclpy.time.Time) -> None:
        ...
        marker.header.stamp = now.to_msg()
        marker.header.frame_id = self.frame_id
        marker.ns = "interceptor_mpc_path"
        marker.id = 0
